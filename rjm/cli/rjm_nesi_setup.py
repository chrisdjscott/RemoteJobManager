...
The default behaviour (equivalent to running with option :code:`--config`) is to create a new Globus guest collection
...
If run with the :code:`--restart` flag, this script will check for an existing funcX endpoint on NeSI. If it finds one, it
will restart it (or start it if it wasn't already running). If the user does not have a funcX endpoint on NeSI, a new one
will be created and started. No configuration values are written to the config file. This option is best used after you have
already run the default program once (to setup the Globus and funcX endpoints and write the config file) and you need to
restart your funcX endpoint for some reason, e.g. it got into a bad state due to network issues on NeSI.
